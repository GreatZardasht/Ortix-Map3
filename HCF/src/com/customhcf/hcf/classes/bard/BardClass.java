...
public class BardClass
    ...
    private final EffectRestorer bardRestorer;
    ...
    public BardClass(HCF plugin) {
        ...
        this.plugin = plugin;
        this.bardRestorer = new EffectRestorer(plugin);
        ...
        this.bardEffects.put(Material.BLAZE_POWDER, new BardEffect(50, new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 120, 0), new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 100, 0)));
            ...
            public void run() {
                final ItemStack held = player.getItemInHand();
                    ...
                    if (energy != 0 && energy != this.lastEnergy && (energy % 10 == 0 || this.lastEnergy - energy - 1 > 0 || energy == 100.0)) {
                        ...
                        player.sendMessage(ChatColor.AQUA + BardClass.this.name + " Energy: " + ChatColor.AQUA + energy);
        ...
        if (action == Action.RIGHT_CLICK_AIR || !event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (bardData != null) {
                ...
                if (stack.getAmount() > 1) {
                    ...
                    this.plugin.getTimerManager().spawnTagTimer.setCooldown(player, player.getUniqueId(), 30L, true);
                    HCF.getPlugin().getTimerManager().spawnTagTimer.setCooldown(player, player.getUniqueId(), 15L, true);
                    Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "timer set " + player.getName() + " Combat 15s");
