...
public class Crowbar {
    ...
    public Optional<ItemStack> toItemStack() {
        if (this.needsMetaUpdate) {
            ...
            if (Math.abs((curDurability -= (increment = curDurability / 6.0) * (double)(this.spawnerUses + this.endFrameUses)) - maxDurability) == 0.0) {
                return Optional.absent();
            }
            ItemMeta meta = this.stack.getItemMeta();
            meta.setDisplayName(CROWBAR_NAME);
            meta.setLore(Arrays.asList(String.format(LORE_FORMAT, "Spawner Uses", this.spawnerUses, 1), String.format(LORE_FORMAT, "End Frame Uses", this.endFrameUses, 5)));
