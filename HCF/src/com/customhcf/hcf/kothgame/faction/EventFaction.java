...
extends ClaimableFaction {
    ...
    public String getScoreboardName() {
        return ChatColor.BOLD.toString() + this.getName();
    }

    @Override
    public String getDisplayName(Faction faction) {
        if (this.getEventType() == EventType.KOTH) {
            return (Object)ChatColor.GREEN + this.getName() + " KOTH";
        }
        return (Object)ChatColor.DARK_PURPLE + this.getEventType().getDisplayName();
    }
    ...
    public String getDisplayName2(CommandSender sender)
    {
        if (getEventType() == EventType.PALACE) {
            return ChatColor.LIGHT_PURPLE.toString() + getName() + ' ' + getEventType().getDisplayName();
        }
