...
extends CommandArgument {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (allied.size() >= 1) {
            sender.sendMessage((Object)ChatColor.RED + "Your faction cant have more allies than " + 1 + '.');
            return true;
        }
        if (targetFaction.getAllied().size() >= 1) {
            sender.sendMessage(targetFaction.getDisplayName(sender) + (Object)ChatColor.RED + " has reached their maximum alliance limit, which is " + 1 + '.');
            return true;
        }
        ...
        targetFaction.broadcast(playerFaction.getDisplayName(targetFaction) + (Object)ChatColor.YELLOW + " has sent a request to be " + RELATION.getDisplayName() + (Object)ChatColor.YELLOW + ". Use " + (Object)ChatColor.BLUE + "/faction " + this.getName() + ' ' + playerFaction.getName() + (Object)ChatColor.YELLOW + " to accept.");
        return true;
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        for (Player target : Bukkit.getServer().getOnlinePlayers()) {
            ...
            if (target.equals((Object)player) || !player.canSee(target) || results.contains(target.getName()) || (targetFaction = this.plugin.getFactionManager().getPlayerFaction(target)) == null || playerFaction == targetFaction || playerFaction.getRequestedRelations().get(targetFaction.getUniqueID()) == RELATION || playerFaction.getRelations().get(targetFaction.getUniqueID()) == RELATION) continue;
